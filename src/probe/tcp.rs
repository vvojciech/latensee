...
pub fn build_tcp_syn(src_port: u16, dst_port: u16, target: IpAddr, seq_num: u32) -> Vec<u8> {
    ...
    match target {
        ...
        IpAddr::V6(_dst) => {
            // IPv6 TCP checksum not yet implemented
        }
    }
...
///
/// `buf` starts at the IP header (as returned by raw socket recv).
pub fn parse_tcp_response(buf: &[u8], src_port: u16, dst_port: u16) -> Option<TcpResponseType> {
    if buf.len() < 20 {
        return None;
    }
    ...
    if ip_version != 4 {
        return None; // IPv6 not yet handled
    }
    ...
    match protocol {
        ...
        1 => parse_icmp_time_exceeded(buf, ip_header_len, src_port, dst_port),
...
/// Parse ICMP time-exceeded that embeds our original TCP header.
        ...
        loop {
            ...
            match icmp_sock.recv_from(&mut recv_buf) {
                Ok((n, _addr)) => {
                    ...
                    if parse_tcp_response(received, src_port, port).is_some() {
                        return Ok(Some(rtt));
                    }
                }
                Err(e)
...
mod tests {
    ...
    #[test]
    fn build_tcp_syn_checksum_is_nonzero() {
        let packet = build_tcp_syn(30001, 80, IpAddr::V4(Ipv4Addr::new(1, 1, 1, 1)), 1);
        let tcp = TcpPacket::new(&packet).expect("should parse as valid TCP packet");
        assert_ne!(
            tcp.get_checksum(),
            0,
            "checksum should be computed (non-zero)"
        );
    }
    ...
    fn parse_tcp_response_identifies_time_exceeded_with_matching_tcp() {
        ...
        let result = parse_tcp_response(&buf, 30001, 80);
        assert_eq!(
            result,
    ...
    fn parse_tcp_response_returns_none_for_unrelated_packets() {
        ...
        assert_eq!(parse_tcp_response(&buf, 30001, 80), None);
        ...
        assert_eq!(parse_tcp_response(&buf2, 30001, 80), None);

        // Buffer too short
        assert_eq!(parse_tcp_response(&[0u8; 10], 30001, 80), None);

        // Unknown protocol (UDP = 17)
        let mut buf3 = vec![0u8; 40];
        buf3[0] = 0x45;
        buf3[9] = 17;
        assert_eq!(parse_tcp_response(&buf3, 30001, 80), None);
    }
